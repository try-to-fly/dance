  ...
  return (
    <div className="space-y-2">
      {sections.map((section) => (
        <Card
          key={section.title}
          className="rounded-[16px] border-border/70 bg-background/70 shadow-none"
        >
          ...
          <CardContent className="space-y-1.5 px-3 pb-3">
            {section.items.map((item) => (
              <div
                ...
                className="grid grid-cols-[minmax(0,auto)_minmax(0,1fr)] items-start gap-2 rounded-[14px] border border-border/60 bg-muted/20 px-2 py-1.5"
              >
