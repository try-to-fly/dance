import { ClipboardEntry, ContentMetadata, ContentSubType } from '../../types/clipboard';
...
export const getEntrySubType = (entry?: ClipboardEntry | null): ContentSubType => {
  ...
  const subtype = String(entry.content_subtype).toLowerCase();
  const knownSubTypes: ContentSubType[] = [
    'plain_text',
    'url',
    'ip_address',
    'email',
  ...
  return knownSubTypes.includes(subtype as ContentSubType)
