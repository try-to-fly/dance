...
v1.0 交付了 Dance 的本地开发者剪贴板核心能力：稳定 capture lifecycle、Rust authoritative analysis、统一 developer previews、可解释的本地 retrieval，以及 rebuild safety 和 release validation gates。
...
- [x] **Phase 3: Unified Developer Previews** - Render supported developer content through one consistent preview pipeline across raw and semantic representations. (completed 2026-03-29)
- [x] **Phase 4: Search Quality & Retrieval** - Deliver responsive indexed, filtered, fuzzy, and ranked retrieval for large local clipboard histories. (completed 2026-03-29)
- [x] **Phase 5: Rebuild Safety & Release Gates** - Preserve history across upgrades and prevent regressions with rebuild tooling and packaged validation. (completed 2026-03-29)
...
4. User can inspect code and command items in a developer-oriented view with preserved formatting and language or shell hints, and use a temporary local workbench when needed.
...
- [x] 03-02-PLAN.md — 将 Clipboard list 收口为固定高度的两层语义摘要
- [x] 03-03-PLAN.md — 落地本地 URL 结构卡与默认无远端依赖的 detail contract
- [x] 03-04-PLAN.md — 打磨 Color detail renderer 的 swatch 与开发友好格式
- [x] 03-05-PLAN.md — 闭合 code/command 本地 workbench 的临时编辑与复制合同
...
- [x] 04-01-PLAN.md — 建立 Rust authoritative retrieval、统一 query model、retrieval snippet/match reason 与 thin-client filter UI
...
- [x] 05-01-PLAN.md — 把 search rebuild 串进现有 rebuild 入口，并为 release/test-build 加入打包前验证门禁
...
| 3. Unified Developer Previews               | 5/5            | Complete | 2026-03-29 |
| 4. Search Quality & Retrieval               | 1/1            | Complete | 2026-03-29 |
| 5. Rebuild Safety & Release Gates           | 1/1            | Complete | 2026-03-29 |
