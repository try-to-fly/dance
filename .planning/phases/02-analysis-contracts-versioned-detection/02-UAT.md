...
- truth: "复制一个 HTTPS URL 后，条目会以 URL 类型显示。打开详情可看到 URL 的结构化信息，例如协议、主机和路径，并且可以执行打开链接操作。"
  ...
  - path: "src/components/DetailView/DetailPreviewContract.test.tsx"
    ...
    missing:
  - "让 URL 条目默认保持 `url_card` 主视图，把远端 resolved JSON/文本/媒体视为备用视图。"
  - "禁止 URL 条目在 JSON renderer 上回退使用原始 URL 字符串。"
  - "补一条覆盖 `url + remote json` 场景的详情契约回归测试。"
...
- truth: "复制合法 JSON 后，详情页会进入 JSON 结构化预览，同时仍保留 Raw 视图可切换查看原始内容。"
  ...
  - path: "src/components/DetailView/DetailView.test.tsx"
    ...
    missing:
  - "保留 JSON 条目的 raw-only 备用视图入口，满足主 JSON + Raw 切换合同。"
  - "给详情左列建立稳定的纵向滚动容器，让长内容和下方卡片都可滚动访问。"
  - "把 JsonRenderer 代码区改成显式高度布局，并补代码视图与长内容滚动回归测试。"
